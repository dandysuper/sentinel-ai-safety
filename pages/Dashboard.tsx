    ...
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-gradient-to-br from-emerald-50 to-white border border-emerald-100 rounded-xl p-6 shadow-sm flex items-center justify-between">
          <div>
            <h3 className="text-emerald-800 font-semibold uppercase tracking-wider text-xs mb-1">Current Safest Model</h3>
            <div className="text-2xl font-bold text-slate-900">{topSafe?.name || (isLoading ? 'Loading...' : '-')}</div>
        ...
        <div className="bg-gradient-to-br from-rose-50 to-white border border-rose-100 rounded-xl p-6 shadow-sm flex items-center justify-between">
           <div>
            <h3 className="text-rose-800 font-semibold uppercase tracking-wider text-xs mb-1">Lowest Safety Score</h3>
            <div className="text-2xl font-bold text-slate-900">{mostRisky?.name || (isLoading ? 'Loading...' : '-')}</div>
        ...
        <div className="w-full lg:w-64 space-y-6 flex-shrink-0">
          <div className="bg-white rounded-lg border border-slate-200 p-5 shadow-sm sticky top-24">
            ...
            <div className="space-y-4">
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Search Model</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Access Type</label>
              ...
              <div>
                <label className="block text-xs font-medium text-slate-500 mb-1.5">Min. Safety Score: {filters.minScore}</label>
                <input 
                  type="range" 
                  min="0" 
                  max="65" 
        ...
        <div className="flex-1 bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200 text-xs uppercase tracking-wider text-slate-500">
                  <th className="p-4 font-semibold cursor-pointer hover:text-slate-700" onClick={() => handleSort('rank')}>Rank</th>
                  <th className="p-4 font-semibold cursor-pointer hover:text-slate-700" onClick={() => handleSort('name')}>Model Name</th>
                  <th className="p-4 font-semibold">Type</th>
                  <th className="p-4 font-semibold cursor-pointer hover:text-slate-700 text-right" onClick={() => handleSort('aggregateScore')}>Safety Score</th>
                  <th className="p-4 font-semibold text-right">7-Day Trend</th>
                </tr>
              </thead>
          ...
          {filteredModels.length === 0 && (
              <div className="p-12 text-center text-slate-500">
                 {isLoading ? 'Loading leaderboard...' : 'No models found matching your criteria.'}
              </div>
           )}
        </div>
      </div>
